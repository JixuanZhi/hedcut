...
void Hedcut::sample_initial_points(cv::Mat & img, int n, std::vector<cv::Point2d> & pts)
{
    ...
    cv::Mat ditherimg=img.clone();
     //Floyd-Steinberg Dithering method
    for (int r= img.size().height-1; r>=0; r--)
    {
        for(int c=0; c< img.size().width; c++)
        {
            cv::Point pix(r,c);
            unsigned int oldpix =ditherimg.at<uchar>(r,c);
            unsigned int newpix = (oldpix > 128) ? 255 : 0;
            ditherimg.at<uchar>(r,c)= newpix;
            int error  = oldpix - newpix;
            
            ditherimg.at<uchar>(r,c)= newpix;
            
            if(c+1<img.size().width)
            ditherimg.at<uchar>(r,c+1) += 7.0/16.0 *error;
            
            if(r-1>=0)
            ditherimg.at<uchar>(r-1,c) += 5.0/16.0 *error;
            if(r-1>=0 && c-1 >=0)
            ditherimg.at<uchar>(r-1,c-1) += 3.0/16.0 * error;
            if(r-1>=0 && c+1 <img.size().width)
            ditherimg.at<uchar>(r-1,c+1) += 1.0/16.0 * error;
        }
    }
    
    //cv::imshow("dd", ditherimg);
	...
	while (count < n)
	{
		...
		unsigned int value = ditherimg.at<uchar>(r, c); //black:0, white:255
		//float gr = fabs(rng_gaussian.gaussian(0.8));
		if ( value==0  && visited.at<uchar>(r, c) ==0) //reject white pixel.
		{
			count++;
