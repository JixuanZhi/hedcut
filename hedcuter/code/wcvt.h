...
struct VorCell
{
    ...
    float closest;  //the size of voronoi cell
    float farthest;
};
...
private:
    ...
    static float move_site_GPU(VorCell & cell);   
	...
	inline float move_sites(cv::Mat &  img, VorCell & cell)
	{
        ...
        if(vorsize) //compute the cell size
        {
            float distance;
            ...
            for (auto& c : cell.edge)
            {
                if(c.x==new_pos.x && c.y==new_pos.y) continue;
                    distance = sqrt(pow(c.x-new_pos.x,2.0f)+pow(c.y-new_pos.y,2.0f));
                if ( closest > distance ) {
                    closest = distance;
                }
       ...
       // std::cout<<total<<"and"<<maxtotal<<"old"<<cell.closest<<std::endl;
        
            cell.closest = closest * total/maxtotal;
            cell.farthest = farthest * total/maxtotal;
        }
        //done
		return dist;
	}
