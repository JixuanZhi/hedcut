...
int main(int argc, char ** argv)
{
	...
	if (argc < 2)
	{
		cout << " Usage: " << argv[0] << " [-n #_of_disks -radius disk_radius -uniform_radius -iteration #_of_CVT_iterations -maxD max_CVF_site_displacement -vorsize #discs size of vor cell] image_file_name" << endl;
		return -1;
	}
	...
	for (int i = 1; i < argc; i++)
	{
		if (argv[i][0] == '-')
		{
            ...
            else if (string(argv[i]) == "-vorsize" && i + 1 < argc) hedcut.vorsize= true;
            else
