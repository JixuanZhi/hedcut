...
void CVT::vor(cv::Mat &  img)
{
    ...
    if(vorsize)  //check the edge of voronoi cell
    {
        for (int x = 0; x < res.height; x++)
        {
            for (int y = 0; y < res.width; y++)
            {
                ...
                {
                    int nx = x + dx;
                    if (nx < 0 || nx >= res.height) continue;
                    for (int dy = -1; dy <= 1; dy++) //y is column
                    {
                        if (dx == 0 && dy == 0) continue; //itself...
                    
                        int ny = y + dy;
                            if (ny < 0 || ny >= res.width) continue;
                        ushort rootid_ne = root.at<ushort>(nx,ny);
                        if (rootid != rootid_ne)
                        {
                            this->cells[rootid].edge.push_back(cv::Point(x,y));
                        }
                    }//end for dy
                }//end for dx
...
void CVT::compute_weighted_cvt(cv::Mat &  img, std::vector<cv::Point2d> & sites)
{
	...
	for (int i = 0; i < site_size; i++)
	{
        ...
        this->cells[i].closest= FLT_MAX;
        this->cells[i].farthest= FLT_MIN;
	}
